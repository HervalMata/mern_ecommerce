...
class UsuarioController {
    ...
    update(req, res, next) {
        ...
        Usuario.findById(req.payload.id).then(usuario => {
            ...
            return usuario.save().then(() => res.json ({
                usuario: usuario.enviarAuthJSON()
            })).catch(next);;
        }).catch(next);
    }
    ...
    completeRecovery(req, res, next) {
        ...
        Usuario.findOne({"recovery.token": token}).then((usuario) => {
            if (!usuario) return res.render('recovery', { error: "Usuário não encontrado", success: null});
            usuario.finalizarTokenRecuperacaoSenha();
            usuario.setSenha(password);
            return usuario.save().then(() => {
                return res.render('recovery/store', {
                    error: null, success: "Senha alterada com sucesso. Tente novamente fazer login", token: null
                });
            }).catch(next);
        }).catch(next);
    }
